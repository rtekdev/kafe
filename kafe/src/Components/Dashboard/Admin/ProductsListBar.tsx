...
const ProductsListBar: React.FC<{
	numOfIndexses: number;
	onChangeListIndex: (index: number) => void;
	numOfProducts: number;
}> = ({ numOfIndexses, onChangeListIndex, numOfProducts }) => {
	const [currentIndex, setCurrentIndex] = useState<number>(1);

    console.log(numOfIndexses)
	...
	return (
		<Stack direction="horizontal" className="users__bar">
			...
			<p>
				...
				{Math.min(currentIndex * 5, numOfProducts)} of {numOfProducts}
			</p>
		</Stack>
	);
};

export default ProductsListBar;
